...
public class TreeMap<K, V>
     ...
     */
    //如果TreeMap继承于Map接口,那么采用putAll方法
     ...
     * @throws NullPointerException if the specified map is null
     */
    //如果TreeMap继承于SortedMap接口,那么采用buildFromSorted方法
    ...
    public void putAll(Map<? extends K, ? extends V> map) {
        ...
        //判断map是否继承了SortedMap,不是则采用AbstractMap的putAll
        if (size == 0 && mapSize != 0 && map instanceof SortedMap) {
            Comparator<?> c = ((SortedMap<?, ?>) map).comparator();
            if (c == comparator || (c != null && c.equals(comparator))) {
                ++modCount;
                //采用SortedMap的buildFromSorted方法
                try {
     ...
     */
    //用来计算二叉树的层数,根据红黑树的特性.
    //假设根节点的层数为1,那么它的下1层节点节点数为(层数 + 1) * 2
    //满足m = (m + 1) * 2,右边的m代表该层的节点数
